...
const formatUtcToLocal = (utcTimestampString) => {
    if (!utcTimestampString) return ''; // Handle null or undefined timestamps
    try {
        ...
        return utcTimestampString; // Fallback to raw string on error
    }
};
        ...
        <div className="flex-1 overflow-y-auto px-5 pt-2 pb-5 space-y-3 custom-scrollbar">
          {sortedBookings.length > 0 ? (
            sortedBookings.map((b) => (
              <div key={b.id} className="bg-black/30 border border-slate-800 rounded-lg p-3 hover:border-blue-700 transition-all">
                ...
                <div className="border-t border-slate-800 pt-2 mt-3 text-xs text-slate-500 font-mono flex flex-col gap-1">
                  <div className="flex items-center gap-1.5"><Fingerprint className="h-3 w-3" /><span>ID: {b.id}</span></div>
...
const RoomManagement = ({ rooms, onBack, onSave, onDelete, isLoading }) => {
  ...
  const handleFormSubmit = (e) => {
    ...
    formData.append('location', currentRoom.location); // Add location to form data
      ...
      <div className="space-y-6 p-2">
        ...
        <form onSubmit={handleFormSubmit} className="space-y-5">
            <LabeledInput label="Location" required>
                <Input value={currentRoom.location} onChange={(e) => setCurrentRoom({ ...currentRoom, location: e.target.value })} required placeholder="e.g., Main Office, Building B" />
            </LabeledInput>
...
const AdminPanel = () => {
  ...
  const initialNewBooking = { room_id: "", room_name: "", subject: "", organizer_email: "", start_datetime: "", end_datetime: "", internal_participants: 1, external_participants: 0, total_participants: 1, meeting_type: "in-person", attendee_emails: "" };
 ...
 const handleApiCall = useCallback(async (apiAction, loadingKey, successTitle) => {
    ...
    setError(null);
    try {
      const result = await apiAction(); // Execute the passed function
  ...
  const calendarEvents = useMemo(() => {
    return data.bookings
      ...
      .map(b => {
        ...
        return {
          id: b.event_id,
          title: b.subject, // MODIFIED: Title is now just the subject
          start: b.start_datetime,
          end: b.end_datetime,
          extendedProps: { ...b, location }, // Pass all booking data here
  ...
  const handleEventClick = (clickInfo) => {
    ...
    setEmailForm({
      subject: `Regarding your booking: ${booking.subject}`,
      message: `Dear ${booking.organizer_email.split("@")[0]},\n\nWe need to discuss your booking for "${booking.subject}" in room ${booking.room_name} (${booking.location}) scheduled for ${formatDateTime(booking.start_datetime)}.\n\nBest regards,\nAdmin Team`,
    });
    setSideboxView("email");
  };

  const handleCreateBooking = async () => {
    const bookingPayload = { ...newBooking, total_participants: parseInt(newBooking.internal_participants) + parseInt(newBooking.external_participants) };
  ...
  const handleDeleteBooking = async (eventId) => {
    ...
    const apiAction = () => deleteBookingAdmin(eventId);
...
const handleAdminLoginSubmit = async (e) => {
    ...
    } catch (err) {/* error is already handled by handleApiCall */}
  };
  ...
  // NEW: Custom function to render event content
  const renderEventContent = (eventInfo) => {
    ...
    return (
      <div className="p-1.5 text-xs text-white overflow-hidden h-full flex flex-col">
        ...
        <div className="flex-grow" /> {/* Pushes details to the bottom */}
                ...
                <CardContent className="p-6">
                  {loading.all ? <div className="flex justify-center items-center h-[75vh]"><Loader2 className="h-16 w-16 animate-spin" /></div>
                   ...
                   :<FullCalendar
                    ...
                    eventContent={renderEventContent} // MODIFIED: Use custom render function
                    height="auto"
                      ...
                      <div className="flex flex-col sm:flex-row gap-3">
                       ...
                       {loading.all ? <div className="flex justify-center py-4"><Loader2 className="h-6 w-6 animate-spin" /></div>
                        : upcomingBookings.map(b => (
                          <div key={b.id} onClick={() => handleEventClick({ event: { extendedProps: b }})} className="text-white bg-gradient-to-br from-gray-950/85 to-indigo-950/75 border border-blue-800/60 rounded-xl p-4 cursor-pointer">
                             <h3 className="font-bold text-blue-200">{b.subject}</h3>
                             <p><MapPin className="inline h-4 w-4" /> {b.room_name}</p>
                             <p><Clock className="inline h-4 w-4" /> {formatDateTime(b.start_datetime)}</p>
                          </div>
                       ))}
                    </CardContent>
                  </>
                  ...
                  <CardHeader><Button onClick={() => setSideboxView("list")} variant="ghost" size="icon" className="text-white"><ArrowLeft /></Button><CardTitle className="text-slate-100">NEW BOOKING</CardTitle></CardHeader>
                    <CardContent className="space-y-5 flex-1 overflow-y-auto custom-scrollbar">
                      ...
                      <Input
                        ...
                        type="number"
                        value={newBooking.internal_participants}
                        onChange={(e) => setNewBooking({
                          ...newBooking,
                          internal_participants: e.target.value,
                          total_participants: parseInt(e.target.value || 0) + parseInt(newBooking.external_participants || 0)
                        })}
                        placeholder="Internal Participants"
                        min="0"
                      />
                      <Input className="text-slate-900 placeholder:text-slate-400" type="number" value={newBooking.external_participants} onChange={(e) => setNewBooking({...newBooking, external_participants: e.target.value})} placeholder="External Participants" min="0" />
                      <Textarea className="text-slate-900 placeholder:text-slate-400" value={newBooking.attendee_emails} onChange={(e) => setNewBooking({...newBooking, attendee_emails: e.target.value})} placeholder="Attendee Emails (comma-separated)" rows={3} />
                     ...
                     <CardContent className="space-y-5 flex-1">
                      <div className="p-4 rounded-xl border text-white">
                        ...
                        <div className="flex flex-col sm:flex-row sm:justify-between gap-3">
                          ...
                          <LoadingButton variant="destructive" onClick={() => handleDeleteBooking(selectedBooking.event_id)} isLoading={loading.delete} className="w-full sm:w-auto"><Trash2/>CANCEL BOOKING</LoadingButton>
                        </div>
                     </CardContent>
                  </>
                )}
