...
import { BookingSlot, Room, BackendBooking } from "@/types/room";

// Props for ManageBooking component
interface ManageBookingProps {
  onClose: () => void;
  // MODIFICATION: Update onReschedule to accept BookingSlot and organizerEmail (string)
  onReschedule: (booking: BookingSlot, organizerEmail: string) => void;
  allRooms: Room[]; // This prop is correctly passed to ManageBooking
}
...
export const ManageBooking = ({
  ...
  const [step, setStep] = useState<"input" | "loading" | "results" | "error">(
    "input"
  );
  ...
  const handleFindBookings = async () => {
    ...
    setError("");
    try {
      const foundBookings = await fetchBookingsByEmail(email);

      const futureBookings = foundBookings.filter(
        (b) => new Date(b.end.dateTime) > new Date()
      );
  ...
  const handleCancelBooking = async (bookingId: string) => {
    ...
    try {
      await cancelBooking(bookingId, email); // Pass email as organizerEmail
      ...
      setTimeout(() => {
        ...
        if (remainingBookings.length === 0) {
          setError(
            "All your bookings have been managed. You can close this window now."
          );
          setStep("error");
        }
      }, 1500);
  ...
  // New function to handle the actual cancellation for reschedule
  const confirmRescheduleAndRedirect = async () => {
    if (!rescheduleCandidate || !rescheduleCandidate.id || !rescheduleCandidate.organizer?.emailAddress?.address) {
    ...
    try {
        await cancelBooking(rescheduleCandidate.id, rescheduleCandidate.organizer.emailAddress.address);
        // On successful cancellation, trigger the reschedule flow with the original booking data
        // This will pre-fill the BookingForm
        // MODIFICATION: Pass organizerEmail to onReschedule
        onReschedule(rescheduleCandidate, rescheduleCandidate.organizer.emailAddress.address);
        onClose(); // Close the ManageBooking modal

        // Optionally, remove the cancelled booking from the list immediately
        setBookings((prev) => prev.filter(b => b.id !== rescheduleCandidate!.id));

    } catch (err: any) {
        setRescheduleError(err.message || "Failed to cancel original booking for reschedule.");
    } finally {
        setRescheduleProcessing(false);
        setShowRescheduleConfirm(false); // Close the confirmation modal
        setRescheduleCandidate(null); // Clear the candidate
    }
  };
  ...
  const renderBookingCard = (booking: BookingSlot) => {
    ...
    return (
      ...
      >
        ...
        <h4 className="font-bold text-lg text-white">{booking.subject}</h4>
        <div className="text-sm text-blue-200 space-y-2 border-t border-blue-500/20 pt-3">
          <p className="flex items-center gap-2">
            <MapPin className="h-4 w-4 text-blue-400" />
            <span className="font-medium">{booking.location?.displayName}</span>
          </p>
          <p className="flex items-center gap-2">
            <Calendar className="h-4 w-4 text-blue-400" />
            <span className="font-medium">
              {new Date(booking.start.dateTime).toLocaleDateString(undefined, {
                weekday: "long",
                month: "long",
                day: "numeric",
              })}
          ...
          <p className="flex items-center gap-2">
            ...
            <span className="font-medium">
              ...
              -{" "}
              {new Date(booking.end.dateTime).toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit",
              })}
            </span>
        ...
        <div className="flex gap-3 pt-3 border-t border-blue-500/20">
          ...
          <Button
            size="sm"
            onClick={() => {
                setRescheduleCandidate(booking); // Set the booking to be rescheduled
                setShowRescheduleConfirm(true);  // Show the confirmation modal
            }}
            disabled={isProcessing}
            ...
            <CardContent className="p-6 min-h-[250px] flex flex-col justify-center">
              ...
              {step === "results" && (
                <div className="space-y-4">
                  ...
                  {rescheduleError && ( // Display reschedule error here
                    <Alert variant="destructive" className="border-red-500/50 bg-red-900/20 text-red-300">
                      <AlertTriangle className="h-4 w-4" />
          ...
          <Card className="w-full max-w-md gaming-card shadow-2xl shadow-purple-500/20 rounded-2xl animate-fade-in">
            ...
            <CardContent className="p-4 sm:p-6 space-y-4 text-center max-h-[60vh] overflow-y-auto">
              ...
              <p className="text-gray-300">
                at <span className="font-medium text-purple-300">{rescheduleCandidate.location?.displayName}</span>
              </p>
              <p className="text-gray-300">
                on <span className="font-medium text-purple-300">{new Date(rescheduleCandidate.start.dateTime).toLocaleDateString()}</span>
                {" from "}
                <span className="font-medium text-purple-300">{new Date(rescheduleCandidate.start.dateTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                {" to "}
                <span className="font-medium text-purple-300">{new Date(rescheduleCandidate.end.dateTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>?
              </p>
