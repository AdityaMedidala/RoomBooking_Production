...
import { Loader2, Calendar as CalendarIcon, KeyRound, Search, Clock, FilterX, X, Circle, Triangle, Square, Zap, MapPin } from 'lucide-react'; // <-- Added MapPin
...
const Index = () => {
  ...
  const handleApplyFilter = async () => {
      ...
      try {
        ...
        // MODIFIED: Added location to the filter logic
        if (filters.location) {
  ...
  // --- MODIFIED: Added memoized selectors for unique locations and names ---
  const uniqueLocations = useMemo(() => Array.from(new Set(allRooms.map(room => room.location))).sort(), [allRooms]);
  const uniqueRoomNames = useMemo(() => {
      const roomsToList = filters.location ? allRooms.filter(r => r.location === filters.location) : allRooms;
      return Array.from(new Set(roomsToList.map(room => room.name))).sort();
    ...
    <div className="flex items-center gap-4">
      ...
      <main className="container mx-auto px-6 py-10 relative z-10">
        <div className="mb-12 relative filter-box p-8">
          ...
          {/* --- MODIFIED: Filter grid layout and added location dropdown --- */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 items-end relative">
            <div className="space-y-2">
                <Label htmlFor="location" className="text-sm font-medium text-slate-400 flex items-center gap-2"><MapPin className="w-4 h-4" /> Location</Label>
                <select id="location" name="location" value={filters.location} onChange={handleFilterChange} className="bg-slate-800/50 border-slate-700 flex h-11 w-full rounded-lg px-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all">
             ...
             <div className="space-y-2">
                ...
                <select id="roomName" name="roomName" value={filters.roomName} onChange={handleFilterChange} className="bg-slate-800/50 border-slate-700 flex h-11 w-full rounded-lg px-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all" disabled={!uniqueRoomNames.length}>
